...
const LoanTrackingController = {
  getLoanTracking: async (req, res) => {
    try {
      const list = await LoanTrackingModel.findAndCountAll({
        limit: 5,
        offset: 0,
      });
  ...
  createLoanTracking: async (req, res) => {
    const payload = req.body;
    try {
      const loanTracking = await LoanTrackingModel.create({ ...payload });
